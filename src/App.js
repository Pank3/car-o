...
function App() {
  ...
  const fetch_data_from_db = async () => {
    ...
    for (let i = 0; i < all_drivers_db.length; i++) {
      if (all_drivers_db[i].request === true) {
        ...
        ch_driver.couter = ch_driver.couter - 5;


        //Finding passenger
        let current_pass;
        for(let i=0;i<all_passengers_db.length;i++){
          if(all_passengers_db[i].id===ch_driver.requestListOfPassengers.passenger_id){
  ...
  //``````````````````````````Not Necessary`````````````````````````
  // fetch_data_from_db();
  // // console.log(all_drivers_db,all_passengers_db);
  // const data = service.getAllDrivers();
  // setInterval(() => {

  //   fetch_data_from_db();
  //   console.log("Driver Data : ",all_drivers_db);
  ...
  useEffect(() => {
    const timer = setInterval(fetch_data_from_db, 5000);
    // console.log(123);
